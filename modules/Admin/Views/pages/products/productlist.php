											...
											<table id="example" class="table table-striped table-bordered w-100 text-nowrap">
												<thead>
													<tr>
														...
														<th class="wd-15p">Product Qunatity</th>
												...
												<?php $i = 1;
													...
													?>
													<?php foreach ($product as $sid) : ?>
													
														<tr>
															<th><?= $i++ ?></th>
															<td><?= $sid['product_name'] ?></td>
															<td><?= $sid['product_description'] ?></td>
															<?php 
															...
															<?php } else { 
																   foreach ($company as $row) :
																	if (!empty($user_id) && $user_id == $row['company_user_id']) {  
																   ?>
																<td><?= $row['company_name'] ?></td>
																 <?php  } endforeach; }?>
															<td style="text-align: center;"> <img style="width: 139px;" id="blah" src="<?= base_url("uploads/" . $sid['product_image']) ?>" /> </td>

															<td>
																<p class="text-muted"><?php echo (!empty($sid['quantity']) && $sid['quantity']>0) ? '<p style="color:lightgreen;"><b>In Stock</b></p>' : '<p style="color:red;"><b>Out Of Stock</b></p>'; ?></p>
															</td>
															
															<td>

																<a href="<?= base_url('product_edit/' . $sid['id']) ?>" class="mx-2 text-decoration-none text-primary"><i class="fa fa-edit"></i></a>

																<a href="<?= base_url('product_delete/' . $sid['id']) ?>" class="mx-2 text-decoration-none text-danger" onclick="if(confirm('Are you sure to delete  - <?= $sid['product_name'] ?> from list?') !== true) event.preventDefault()"><i class="fa fa-trash"></i></a>
															</td>
														</tr>
													<?php endforeach; ?>
